...
<head>
    ...
    <script>
        function leyenda() {
            var parametros = {
                "taller": <?php echo $_GET['i']; ?>
            };
            $.ajax({
                data: parametros, //datos que se envian a traves de ajax
                url: 'php/infoTaller.php', //archivo que recibe la peticion
    ...
    <script>
        function leyenda() {
            ...
            $.ajax({
                data: parametros,
                url: 'php/disponibilidadTaller.php',
                type: 'post',
    ...
    <script type="text/javascript">
        function confirmation() {
            if (confirm("¿Deseas Inscribirte a este taller?")) {
                return true;
            } else {
                return false;
            }
        ...
        <section class="single-course section-padding-large">
            <div class="container">
                <div class="row">
                    ...
                    $Talleres = $DB_con->prepare("SELECT instructores.*, talleres.* FROM talleres INNER JOIN instructores ON talleres.id_instructor = instructores.id_instructor WHERE id_adminCarrera  =:uid AND status_instructor = :uid2 AND id_taller = :uid3");
                    $Talleres->execute(array(':uid' => $rowAlumno["id_adminCarrera"], ':uid2' => 1, ':uid3' => $id_tallerGet));
                    ...
                    ?>
                        ...
                        <div class="col-xl-9 col-lg-9 col-md-12 col-sm-12 col-12 single-course-content">
                            <div class="single-title">
                                <h1><?php echo $row->nombre_taller; ?> </h1>
                                <div id="leyenda">
                                </div>
                            </div>
                            <div class="single-course-info">
                                <figure class="single-course-images">
                                    <img src="images/talleres/<?php echo $row->imagen_taller; ?>" alt="">
                                </figure>
                            ...
                            <div class="single-course-tab">
                                    ...
                                    <div class="tab-pane fade show active" id="overview" role="tabpanel" aria-labelledby="overview-tab">
                                        <div class="course-overview">
                                            <div class="course-desc">
                                                <!-- <h4 class="course-tab-title">Información del taller</h4> -->
                                                <p class="course-desc-content">
                                                    <?php echo $row->descripcion_taller;
                                                    $cupoTaller =  $row->cupo_taller ?>
                                                </p>
                                            </div>
                                        </div>
                                ...
                                <script>
                                    function listaDeAlumnos() {
                                        var parametros = {
                                            "taller": <?php echo $_GET['i']; ?>,
                                            "cupo": <?php echo $cupoTaller; ?>
                                        };
                                        $.ajax({
                                            data: parametros, //datos que se envian a traves de ajax
                                            url: 'php/listaAlumnos.php', //archivo que recibe la peticion
                                            type: 'post', //método de envio
                                            success: function(response) { //una vez que el archivo recibe el request lo procesa y lo devuelve
                                                $("#lista").html(response);
                                            }
                            ...
                            <div class="listar-nextprevposts">
                                ...
                                <?php
                                // INFORMACION DE LOS TALLERES SIGUIENTES
                                $masTalleres = $DB_con->prepare("SELECT instructores.*, talleres.* FROM talleres INNER JOIN instructores ON talleres.id_instructor = instructores.id_instructor WHERE id_adminCarrera  =:uid AND status_instructor = :uid2 AND id_taller < :uid3 ORDER BY id_taller DESC LIMIT 1");
                                $masTalleres->execute(array(':uid' => $rowAlumno["id_adminCarrera"], ':uid2' => 1, ':uid3' => $id_tallerGet));
                                while ($rowT = $masTalleres->fetch(PDO::FETCH_OBJ)) {
                                ?>
                                    <div class="listar-prevpost">
                                        <a href='taller.php?i=<?php echo $rowT->id_taller; ?>'>
                                            <i class="fa fa-angle-left"></i><span>Taller Anterior</span>
                                ...
                                <?php
                                // INFORMACION DE LOS TALLERES SIGUIENTES
                                $masTalleres = $DB_con->prepare("SELECT instructores.*, talleres.* FROM talleres INNER JOIN instructores ON talleres.id_instructor = instructores.id_instructor WHERE id_adminCarrera  =:uid AND status_instructor = :uid2 AND id_taller > :uid3 ORDER BY id_taller ASC LIMIT 1");
                                $masTalleres->execute(array(':uid' => $rowAlumno["id_adminCarrera"], ':uid2' => 1, ':uid3' => $id_tallerGet));
                                while ($rowT = $masTalleres->fetch(PDO::FETCH_OBJ)) {
                                ?>
