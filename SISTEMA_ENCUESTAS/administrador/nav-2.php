<nav class="sidenav shadow-right sidenav-light">
    <div class="sidenav-menu">
        <div class="nav accordion" id="accordionSidenav">
            <div class="sidenav-menu-heading">Administrador</div>
            <a class="nav-link" href="../../SAC/administrador_general/index.php">
                <div class="nav-link-icon"><i data-feather="home"></i></div>Inicio
            </a>

            <a class="nav-link" href="../../../congreso-master/SAC/administrador_general/carreras/index.php">
                <div class="nav-link-icon"><i data-feather="folder"></i></div>Carreras
            </a>
            <a class="nav-link" href="../administrador/index.php">
                <div class="nav-link-icon"><i data-feather="folder"></i></div>Encuestas
            </a>
            <div class="sidenav-menu-heading">Configuración</div>
            <a class="nav-link" href="../../../congreso-master/SAC/administrador_general/correos.php">
                <div class="nav-link-icon"><i data-feather="folder"></i></div>Correos
            </a>
            

            <a class="nav-link" href="../../../congreso-master/SAC/administrador_general/semestre_grupo.php">
                <div class="nav-link-icon"><i data-feather="folder"></i></div>Semestre y grupo
            </a>


            <div class="sidenav-menu-heading">Perfil</div>
            <a class="nav-link" href="../../../congreso-master/SAC/administrador_general/perfil.php">
                <div class="nav-link-icon"><i data-feather="user"></i></div>
                Perfil
            </a>
            <a class="nav-link" href="../../logout.php">
                <div class="nav-link-icon"><i data-feather="log-out"></i></div>
                Salir
            </a>
        </div>
    </div>
    <div class="sidenav-footer">
        <div class="sidenav-footer-content">
            <div class="sidenav-footer-subtitle">Logeado como:</div>
            <div class="sidenav-footer-title">Administrador</div>
        </div>
    </div>
</nav>