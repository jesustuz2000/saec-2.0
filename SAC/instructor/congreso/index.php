...
if ($contador['cont'] == 1) {
    ...
    // CONTADOR DE ALUMNO (saber si el taller esta lleno)
    $consulta3 = $DB_con->prepare('SELECT COUNT(*) AS contA FROM talleres WHERE id_instructor =:uid');
    $consulta3->execute(array(':uid' => $datosTaller['id_taller']));
    $contadorAlumno = $consulta3->fetch(PDO::FETCH_ASSOC);
    extract($contadorAlumno);
    $contadorAlumnoLleno = $contadorAlumno['contA'];
} else {
    print "<script>window.location='taller.php';</script>";
}
...
if (isset($_GET['delete_id'])) {
    ...
    if ($_GET['delete_id'] == $datosInstructor['id_instructor']) {
        ...
        $elimnarTallerImagen = $DB_con->prepare('SELECT * FROM talleres WHERE id_instructor =:uid');
        ...
        $elimnarTaller = $DB_con->prepare('DELETE FROM talleres WHERE id_instructor =:uid2');
...
<head>
    ...
    <script>
        function listaDeAlumnos() {
            var parametros = {
                "taller": <?php echo $id_taller; ?>,
                "cupo": <?php echo $datosTaller['cupo_taller']; ?>
            };
            $.ajax({
                data: parametros,
                        ...
                        <div class="page-header-content">
                            ...
                            <script type="text/javascript">
                                function confirmarRegistro<?php echo $datosInstructor['id_instructor']; ?>() {
                                    if (window.confirm("¿Seguro que desea eliminar este taller?") == true) {
                                        window.location = "index.php?delete_id=<?php echo $datosInstructor['id_instructor']; ?>";
                                    }
                                }
                            </script>
                        </div>
                            ...
                            <div class="row">
                                <div class="col-xl-9 col-lg-9 col-md-12 col-sm-12 col-12 single-course-content">
                                    <div class="single-title">
                                        <h1><?php echo $datosTaller['nombre_taller']; ?> </h1>
                                        <div id="leyenda">
                                        </div>
                                    </div>
                                    <div class="single-course-info">
                                        <figure class="single-course-images">
                                            <img src="../../../images/talleres/<?php echo $datosTaller['imagen_taller'] ?>" alt="">
                                        </figure>
                                            ...
                                            <div class="teacher-cat">
                                                <ul class="display-flex">
                                                    ...
                                                    <li>
                                                        <div class="teacher-cat-item">
                                                            <span>Área</span>
                                                            <a href="#"><?php echo $datosTaller['lugar_taller'] ?> </a>
                                                        </div>
                                                    </li>
                                                ...
                                                <div class="course-overview">
                                                    <div class="course-desc">
                                                        <p class="course-desc-content">
                                                            <?php echo $datosTaller['descripcion_taller'] ?>
                                                        </p>
                                                    </div>
                                                </div>
