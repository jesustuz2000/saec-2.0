            ...
            <div class="container">
                <div class="courses-content">
                    <div class="row">
                        ...
                        $consultaConcurso = $DB_con->prepare("SELECT instructores.id_adminCarrera, concursos.* FROM concursos INNER JOIN instructores ON concursos.id_instructor = instructores.id_instructor WHERE id_adminCarrera  =:uid AND status_instructor = :uid2 ORDER BY id_concurso ASC");
                        $consultaConcurso->execute(array(':uid' => $rowAlumno["id_adminCarrera"], ':uid2' => 1));

                        while ($row = $consultaConcurso->fetch(PDO::FETCH_OBJ)) {

                            // CONTADOR DE ALUMNOS INSCRITOS AL CONCURSOS
                        ...
                        ?>
                            <div class="col-xl-4 col-lg-4 col-md-6 col-sm-12 col-12">
                                <article class="item">
                                    <div class="item-thumb"> <a href="concurso.php?i=<?php echo $row->id_concurso; ?>"> <img src="images/concursos/<?php echo $row->imagen_concurso; ?>" alt=""> </a> </div>
                                    <div class="info">
                                        <h3 class="title"> <a href="concurso.php?i=<?php echo $row->id_concurso; ?>"><?php echo $row->nombre_concurso; ?></a> </h3>
                                        <p>Modalidad: <?php if ($row->modalidad == 1) {
                                                            echo 'Individual';
                                                        }
                                                        if ($row->modalidad == 2) {
                                                            echo 'Equipos';
                                        ...
                                        <script>
                                            function listaDeAlumnos<?php echo $row->id_concurso; ?>() {
                                                var parametros = {
                                                    "concurso": <?php echo $row->id_concurso; ?>,
                                                    "c": <?php echo $rowAlumno["id_adminCarrera"]; ?>
                                                };
                                                $.ajax({
                                                    data: parametros,
                                                    url: 'php/footerItemConcursos.php',
                                                    type: 'post',
                                                    success: function(response) {
                                                        $("#footerItem<?php echo $row->id_concurso; ?>").html(response);
                                                    }
                                                });
                                            }
                                            setInterval(listaDeAlumnos<?php echo $row->id_concurso; ?>, 1000);
                                        </script>

                                        <div id="footerItem<?php echo $row->id_concurso; ?>">
                                        </div>
                                    </div>
                                </article>
                            </div>
