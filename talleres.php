            ...
            <div class="container">
                <div class="courses-content">
                    <div class="row">
                        ...
                        $Talleres = $DB_con->prepare("SELECT instructores.id_adminCarrera, talleres.* FROM talleres INNER JOIN instructores ON talleres.id_instructor = instructores.id_instructor WHERE id_adminCarrera  =:uid AND status_instructor = :uid2 ORDER BY id_taller ASC");
                        $Talleres->execute(array(':uid' => $rowAlumno["id_adminCarrera"], ':uid2' => 1));
                        ...
                        ?>
                            <div class="col-xl-4 col-lg-4 col-md-6 col-sm-12 col-12">
                                <article class="item">
                                    <div class="item-thumb"> <a href="taller.php?i=<?php echo $row->id_taller; ?>"> <img src="images/talleres/<?php echo $row->imagen_taller; ?>" alt=""> </a> </div>
                                    <div class="info">
                                        <h3 class="title"> <a href="taller.php?i=<?php echo $row->id_taller; ?>"><?php echo $row->nombre_taller; ?></a> </h3>
                                        <p></p>
                                        <script>
                                            function listaDeAlumnos<?php echo $row->id_taller; ?>() {
                                                var parametros = {
                                                    "taller": <?php echo $row->id_taller; ?>,
                                                    "c": <?php echo $rowAlumno["id_adminCarrera"]; ?>
                                                };
                                                $.ajax({
                                                    data: parametros,
                                                    url: 'php/footerItem.php', 
                                                    type: 'post',
                                                    success: function(response) { 
                                                        $("#footerItem<?php echo $row->id_taller; ?>").html(response);
                                                    }
                                                });
                                            }
                                            setInterval(listaDeAlumnos<?php echo $row->id_taller; ?>, 1000);
                                        </script>
                                        
                                        <div id="footerItem<?php echo $row->id_taller; ?>">
                                        </div>
                                    </div>
                                </article>
                            </div>
